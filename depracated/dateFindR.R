...
dateFindR <-
  function(data = NULL,
    ...
    "([1-9]|1[0-2])[\\s- /]+[0-9]{4}"
      ...
      occYr_2$date <- as.character(occYr_2$date) %>% ymd_hms() %>% date()
        # Set as date format...
      ymd_keepers_21$date <- ymd(ymd_keepers_21$date)
      dmy_keepers_22$date <- ymd(dmy_keepers_22$date)
      mdy_keepers_23$date <- ymd(mdy_keepers_23$date)
      my_keepers_24$date <- ymd(my_keepers_24$date)
        # merge these data...
      ...
      datesOut_full$eventDate <- ymd(datesOut_full$date)
