...
There are many ways to get a good starting dataset. For **bees**, we have compiled a bunch of large datasets that we will periodically update and make available. However, we have also made available the code to update many of these public datasets yourself in the [**bee data prep vignette**](articles/dataPrep.html). 

If you are looking to just make your own dataset to feed in, for example, from another taxon. Great! We have some usefull tools below and so does the **bdc** package! Feel free to take you pick at how you compile your starting data but just note that **BeeBDC**, **bdc**, and **coordinateCleaner** like (and often assume) that your data are in a [Darwin Core](https://dwc.tdwg.org) format.
...
Now that we have some kind of dataset put together in Darwin Core format, we can start to think about actually doing some filtering! Keep in mind, as you progress through this vignette, that not all filters might be relevant for your hypothesis! So, thinking about then ones that you apply at the end throughout is probably a useful endeavor. However, do keep in mind that the only thing that it costs you to run every filter is time.
...
```{r 3.0, collapse = TRUE, tidy=FALSE}
...
```{r 3.4, collapse = TRUE, tidy=FALSE}
...
Fix up country names based on common problems above and extract ISO2 codes for occurrences. The below are problems in the global bee dataset, but you may find others in your own dataset. No harm in running the below code and maybe altering it for your own use. Otherwise, don't get too caught up in it :) 


```{r 3.5a, collapse = TRUE, tidy=FALSE}
...
Get country name from coordinates using a wrapper around the `BeeBDC::jbd_country_from_coordinates()` function. Because our dataset is much larger than those used to design **bdc**, we have made it so that you can analyse data in smaller pieces. Additionally, like some other functions in **BeeBDC**, we have implemented parallel operations (using mc.cores = #cores in stepSize = #rowsPerOperation); see '?`BeeBDC::jbd_CfC_chunker()`' for details. 
NOTE: In an actual run you should use scale = "large"

```{r 3.5b, message=FALSE, warning=FALSE, collapse = TRUE, tidy=FALSE}
suppressWarnings(
...
```{r 3.6, collapse = TRUE, tidy=FALSE}
...
```{r 3.7, message=FALSE, warning=FALSE, collapse = TRUE, tidy=FALSE}
...
Many of the large data repositories are happy to store presence and absence records. This is very important to know, because if you leave absence records in your dataset and analyse them as an occurrence record... well, you'll be doing a no-no to say the very least!
...
```{r 3.11, collapse = TRUE, tidy=FALSE}
...
```{r 3.13b, collapse = TRUE, tidy=FALSE}
...
**! MAC**: You might need to install **gnparser** through terminal using [homebrew](https://brew.sh) — brew
			brew tap gnames/gn
...
```
```{r 4.2secret, collapse=TRUE, include=FALSE}
...
```{r 4.2ii, collapse = TRUE, tidy=FALSE}
...
```{r 5.2ii, message=TRUE, warning=FALSE, eval = FALSE, collapse = TRUE, tidy=FALSE}
tempSpace <-
...
```{r 5.3, collapse = TRUE, tidy=FALSE}
...
```{r 5.3iv, eval = FALSE, collapse = TRUE, tidy=FALSE}
...
Flag records that exceed a *coordinateUncertaintyInMeters* threshold in meters.
...
I would like to eventually add checklists for other taxa or get help in this; if anyone is willing!
...
```{r 5.5ii, collapse = TRUE, tidy=FALSE}
...
```{r 5.6ii, eval = FALSE, collapse = TRUE, tidy=FALSE}
check_space %>%
...
```{r 6.1, collapse = TRUE, tidy=FALSE}
...
This will flag records prior to the date selected. The year 1970 is frequently chosen for species distribution modelling work. You may not need to filter old records at all, so think critically about your use. We have chosen 1950 as a lower extreme.

Many other works probably won't need to apply a year filter at all; for example, if you want to make a distribution map for a taxonomic work.
...
```{r 6.4, eval = TRUE, collapse = TRUE, tidy=FALSE}
...
I would encourage you to see '?`BeeBDC::dupeSummary()`' for more details as this function is quite modifiable to user needs.

```{r 7.1, collapse = TRUE, tidy=FALSE}
...
Read in the most-recent duplicates file (generated by `BeeBDC::dupeSummary()`) in order to identify the duplicates of the expert outliers.
...
```{r 8.3, eval = TRUE, collapse = TRUE, tidy=FALSE}
...
```{r 9.1iv, eval=FALSE, fig.fullwidth=TRUE, fig.height=7.5, fig.width=9, collapse = TRUE, tidy=FALSE}
...
```{r 9.2ii, warning=FALSE, eval=TRUE, collapse = TRUE, tidy=FALSE}
BeeBDC::dupePlotR(
  data = beeData,
...
<img src="https://photos.smugmug.com/photos/i-CjrvS3d/0/NWnPWxvP8VGmTwZKjtJbgc5xCzjJwCt4P2PK7Rvqz/X4/i-CjrvS3d-X4.jpg" align="left" />
...
```{r 9.3, fig.width=15, fig.height=9, fig.fullwidth=TRUE, eval=TRUE, collapse = TRUE, tidy=FALSE}
