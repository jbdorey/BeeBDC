...
This R script was written by James B. Dorey, starting on the 2nd of May 2022. The script is intended to clean occurrence data from several sources using several packages; primarily BeeBDC, bdc, and CoordinateCleaner. 
</div>
...
```{r RootPath}
RootPath <- "/Users/jamesdorey/Desktop/Uni/Packages/BeeBDC-website/BeeBDC"
  # Set the working directory
setwd(paste0(RootPath,"/Data_acquisition_workflow"))
```

For the first time that you run BeeBDC, and if you want to use the renv pacakge to manage your 
...
You may need to install gdal on your computer. This can be done on mac by using Homebrew in the terminal and the command "brew install gdal". 

To start out, you will need to install BiocManager, devtools, ComplexHeatmap, and rnaturalearthhires to then install BeeBDC
```{r installPackages}
...
```
  ...
  Set up the directories used by BeeBDC. The RDoc needs to be a path RELATIVE to the RootPath;
  i.e., the file path from which the two diverge.
  
```{r dirMaker}
  BeeBDC::dirMaker(
    RootPath = RootPath,
      # Input the location of the workflow script RELATIVE to the RootPath
    RDoc = "BeeBDC_website.Rmd") %>%
...
###### a. EPEL
Guzman, L. M., Kelly, T. & Elle, E. A data set for pollinator diversity and their interactions with plants in the Pacific Northwest. Ecology n/a, e3927 (2022). <https://doi.org/10.1002/ecy.3927>
...
# 3.0 Initial flags 
Read data back in if needed. OutPath_Intermediate should be created and saved to the global environment by dirMaker (as will several other directories).
...
## 3.2 MissCoords 
Flag occurrences with missing latitude and longitude.
...
Flag and correct records when lat and lon appear to be transposed. We created this chunked version of  bdc::bdc_coordinates_transposed because it is very RAM-heavy using our large bee dataset. However, like many of our other 'jbd_...' functions there are other improvements - e.g., parallel running. 
...
```{r 5.2ii, message=FALSE, warning=FALSE}
tempSpace <-
...
The dateFindR function will search through some other columns in order to find and rescue dates that may not have made it into the correct columns. It wil lfurther update the eventDate, day, month, and year columns where these data were a) missing and b) located in one of the searched columns.
...
## 6.4 Time report 
*Not all of it, just the time pertaining to our precise occurrence records.*
...
```{r 9.1iv, fig.width=9, fig.height=7.5, fig.fullwidth=TRUE}
...
knitr::include_graphics("images/ChordDiagram.pdf")
