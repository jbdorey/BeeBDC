...
#'
#' A small bee occurrence dataset with flags generated by BeeDC which can be used to run the 
#' example script and to test functions. For data types, see [BeeDC::ColTypeR()].
#'
#' @docType data
#'
...
#'  \item{coordinates_transposed}{A column indicating if coordinates were identified as being transposed by the function BeeDC::jbd_Ctrans_chunker() where FALSE == transposed.}
...
#'  \item{.occurrenceAbsent}{A flag generated by [BeeDC::flagAbsent()] where FALSE == occurrences marked as "ABSENT" in the "occurrenceStatus" column}
#'  \item{.unLicensed}{A flag generated by [BeeDC::flagLicense()] where FALSE == those occurrences protected by a restrictive license.}
#'  \item{.GBIFflags}{A flag generated by [BeeDC::GBIFissues()] where FALSE == an occurrence with user-specified GBIF issues to flag.}
...
#'  \item{.sequential}{A flag generated by [BeeDC::diagonAlley()] where FALSE == records that are possibly the result of fill-down errors in sequence.}
...
#'  \item{.duplicates}{A flag generated by [BeeDC::dupeSummary()] where FALSE == occurrences identified as duplicates. There will be an associated kept duplicate (.duplictes == TRUE) for all duplicate clusters.}
