# Description of the BeeDC toy dataset bees3sp.csv written by James B. Dorey on the 
# 16th of March 2023

#' A flagged dataset of 105 random bee occurrence records from the three species
#' 
#' The included species are: 
#' "Agapostemon tyleri Cockerell, 1917", "Centris rhodopus Cockerell, 1897", and 
#' "Perdita octomaculata (Say, 1824)"
#'
#' A small bee occurrence dataset with flags generated by BeeDC used to run example script and test 
#' functions. For data types, see [BeeDC::colTypeR()].
#'
#' @docType data
#'
#' @usage BeeDC::beesFlagged
#'
#' @format An object of class \code{"tibble"}
#' \describe{
#'  \item{database_id}{Occurrence code generated in bdc or BeeDC}
...
#'  \item{verbatimIdentification}{A string representing the taxonomic identification as it appeared in the original record.}
...
#'  \item{verbatimScientificName}{entry}
...
#'  \item{hasCoordinate}{entry}
#'  \item{hasGeospatialIssues}{entry}
#'  \item{isDuplicateOf}{entry}
#'  \item{occurrenceYear}{entry}
#'  \item{id}{entry}
#'  \item{duplicateStatus}{entry}
...
#'  \item{dataSource}{BeeDC assigned source of the data. Often written when the data is formatted by a BeeDC::xxx_readr function or similar.}
...
#'  \item{coordinates_transposed}{A column indicating if coordinates were tansposed by [BeeDC::jbd_Ctrans_chunker()] where FALSE == transposed.}
#'  \item{.coordinates_country_inconsistent}{A flag generated by [BeeDC::jbd_coordCountryInconsistent()] where FALSE == an occurrence where the country name and coordinates did not match.}
#'  \item{.occurrenceAbsent}{A flag generated by [BeeDC::jbd_flagAbsent()] where FALSE == occurrences marked as "ABSENT" in the "occurrenceStatus" column}
#'  \item{.unLicensed}{A flag generated by [BeeDC::jbd_flagLicense()] where FALSE == those occurrences protected by a restrictive license.}
#'  \item{.GBIFflags}{A flag generated by [BeeDC::jbd_GBIFissues()] where FALSE == an occurrence with user-specified GBIF issues to flag.}
...
#'  \item{.invalidName}{A flag generated by [BeeDC::HarmoniseR()] where FALSE == occurrences whose scientificName did not match the Discover Life taxonomy.}
...
#'  \item{.sequential}{A flag generated by [BeeDC::jbd_diagonAlley()] where FALSE == records that are possibly the result of fill-down errors in sequence.}
...
#'  \item{.uncertaintyThreshold}{A flag generated by [BeeDC::coordUncerFlagR()] where FALSE == occurrences that did not pass a user-specified threshold in the "coordinateUncertaintyInMeters" column.}
#'  \item{countryMatch}{A column made by [BeeDC::countryOutlieRs()]. Summarises the occurrence-level result: where the species is not known to occur in that country (noMatch), it is known from a bordering country (neighbour), or it is known to occur in that country (exact).}
#'  \item{.countryOutlier}{A flag generated by [BeeDC::countryOutlieRs()] where FALSE == occurrences the do not occur in a country that concurs with the Discover Life country checklist OR an adjacent country.}
#'  \item{.sea}{A flag generated by [BeeDC::countryOutlieRs()] where FALSE == occurrences that are in the ocean.}
#'  \item{.summary}{A flag generated by [BeeDC::summaryFun()] where FALSE == occurrences flagged as FALSE in any of the .flag columns. In this example it excludes flags in the ".gridSummary", ".lonFlag", ".latFlag", and ".uncer_terms" columns.}
...
#'  \item{.duplicates}{A flag generated by [BeeDC::jbd_dupeSummary()] where FALSE == occurrences identified as duplicates. There will be an associated kept duplicate (.duplictes == TRUE) for all duplicate clusters.}

#' }
#' @references This data set was created by generating a random subset of 105 rows from the full BeeDC dataset from the publication:
