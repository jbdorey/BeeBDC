...
attr_builder <- function(path_i, occ_input){
  require(EML, tibble, lubridate, stringr)
  require(dplyr)
  require(xml2)
  ...
  if(grepl("/data.csv", path_i) == "TRUE"){
    ...
    galahDL_i <- gsub("/data.csv", "/galah_DL_info.csv", path_i) %>% 
      readr::read_csv(n_max = 1, col_types = cols(.default = col_character())) 
  ...
  if(grepl("occurrence_raw.csv", path_i) == "TRUE"){
    ...
    Attributes_i <- tibble::tibble(dataSource = paste("iDigBio_", citations_i$.[2] %>% 
                                                        str_match_all("[A-Za-z]+") %>% unlist() %>% 
                                                        dplyr::last(), sep = ""),
                                   alternateIdentifier =    path_i %>% str_match_all("[a-zA-Z0-9]+-[a-zA-Z0-9]+-[a-zA-Z0-9]+-[a-zA-Z0-9]+-[a-zA-Z0-9]+") %>%
                                     unlist(),
                                   title = citations_i$.[1], 
                                   ...
                                   dateStamp = citations_i$.[3] %>% 
                                     str_match("[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}") %>%
                                     as.character(),
                                   doi = "Use downloadLink for iDigBio",
                                   downloadLink = paste("http://s.idigbio.org/idigbio-downloads/", 
                                                        path_i %>% str_match_all("[a-zA-Z0-9]+-[a-zA-Z0-9]+-[a-zA-Z0-9]+-[a-zA-Z0-9]+-[a-zA-Z0-9]+"),
                                                        ".zip", sep = ""),
  ...
  if(grepl("occurrences.csv", path_i) == "TRUE"){
    ...
    Attributes_i <- tibble::tibble(dataSource = paste("SCAN_", 
                                   ...
                                   downloadLink = "Scan does not provide a download link — ask Neil about if I can create a download url.",
