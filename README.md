...
- `idMatchR()` This function attempts to match database_ids from a prior
  **bdc** or **BeeBDC** run in order to keep this column somewhat
  consistent between iterations. However, not all records contain
  sufficient information for this to work flawlessly.
...
- `flagRecorder()` This function is used to save the flag data for your
  occurrence data as you run the **BeeBDC** script. It will read and
  append existing files, if asked to. Your flags should also be saved in
  the occurrence file itself automatically.
...
- `taxadbToBeeBDC()` Uses the **taxadb** to download a species taxonomy
  ...
  that can then be exported as a .csv or into the **R** environment to
  be be fed directly into `BeeBDC::harmoniseR()`. This means that the
  taxonomy from ANY taxon can be used. See also `BeeBDC::beesTaxonomy()`
...
- `jbd_coordinates_precision()` This function flags occurrences where
  ...
  The **BeeBDC** approach saves occurrences that may have had terminal
  zeros rounded in one coordinate column.
...
- `jbd_create_figures()` Creates figures (i.e., bar plots, maps, and
  ...
  the bdc and **BeeBDC** packages. Works like
  `bdc::bdc_create_figures()`, but it allows the user to specify a save
  path.
